-- prepares a MySQL server for the eServices-KWL app

CREATE DATABASE IF NOT EXISTS eservices_dev_db;

CREATE USER IF NOT EXISTS 'eservices_dev' @'localhost' IDENTIFIED BY 'eservices_pwd';

GRANT ALL PRIVILEGES ON `eservices_dev_db`.* TO 'eservices_dev' @'localhost';

GRANT SELECT ON `performance_schema`.* TO 'eservices_dev' @'localhost';

FLUSH PRIVILEGES;

use eservices_dev_db;
